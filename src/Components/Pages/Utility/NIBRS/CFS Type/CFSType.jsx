...
import CFS_Type_Add_Up from './CFSType_Add_Up';
...
const CFS_Type = () => {
    ...
    const get_Data_List = () => {
        ...
        fetch_Post_Data('CFSType/GetData_CFSType', val)
            .then((res) => {
                if (res) {
                ...
                <div style={{ position: 'absolute', top: 0, right: 40 }}>

                    {
                        pageStatus === "1" ?
                            EffectiveScreenPermission ? EffectiveScreenPermission[0]?.ChangeOK ?
                                <Link to="/ListManagement?page=CFS Type" data-toggle="modal" data-target="#CFSModal" onClick={(e) => { setEditValue(e, row); }}
...

                    {
                        EffectiveScreenPermission ? EffectiveScreenPermission[0]?.DeleteOK ?
                            pageStatus === "1" ?
                                < Link to="/ListManagement?page=CFS Type" data-toggle="modal" data-target="#ConfirmModal" onClick={(e) => { setSingleTypeId(row.CFSTypeID); setIsActive('0'); setConfirmType("InActive") }}
                                    className="btn btn-sm  text-white px-1 py-0 mr-1" style={{ background: "#ddd" }}>
                                    <i class="fa fa-toggle-on" style={{ color: "green" }} aria-hidden="true"></i>
                                </Link>
                                :
                                <Link to="/ListManagement?page=CFS Type" data-toggle="modal" data-target="#ConfirmModal" onClick={(e) => { setSingleTypeId(row.CFSTypeID); setIsActive('1'); setConfirmType("Active") }}
                                    className="btn btn-sm  text-white px-1 py-0 mr-1" style={{ background: "#ddd" }}>
    ...
    const setEditValue = (e, row) => {
        setUpdateStatus(updateStatus + 1); setSingleTypeId(row.CFSTypeID)
        setModal(true)
    ...
    return (
        <>
                    ...
                    <div className="row mt-2">
                        <div className="col-12 ">
                            <div className="bg-green text-white py-1 px-2 d-flex justify-content-between align-items-center">
                                <p className="p-0 m-0">CFS Type</p>
                                {
                                    pageStatus === '1' ?
                                        EffectiveScreenPermission ? EffectiveScreenPermission[0]?.AddOK ?
                                            <Link to="/ListManagement?page=CFS Type" className="text-white" onClick={setStatusFalse}
                    ...
                    <div className="row">
                        <div className="col-5">
                            <input type="text" onChange={(e) => {
                                ...
                                const result = Filter(DataList, e.target.value, searchValue2, filterTypeIdOption, 'CFSTypeCode', 'Description')
                        ...
                        <div className="col-5">
                            <input type="text" onChange={(e) => {
                                ...
                                const result = Filter(DataList, searchValue1, e.target.value, filterTypeDescOption, 'CFSTypeCode', 'Description')
                                setFilterDataList(result)
                            }}
            ...
            <CFS_Type_Add_Up {...{ singleTypeId, status, get_Data_List, DataList, modal, setModal, updateStatus }} />
            <ConfirmModal func={UpdActiveDeactive} confirmType={confirmType} />
        </>
    )
}

export default CFS_Type