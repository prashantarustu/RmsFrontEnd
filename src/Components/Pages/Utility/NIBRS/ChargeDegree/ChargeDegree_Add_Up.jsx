...
const ChargeDegree_Add_Up = (props) => {
    const { singleTypeId, status, get_Data_List, DataList, modal, setModal, updateStatus } = props
    const [agencyData, setAgencyData] = useState([])
    const [Editval, setEditval] = useState();

    const [value, setValue] = useState({
        'ChargeDegreeCode': '', 'AgencyCode':'','Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 'ChargeDegreeID':''
    });
    ...
    const GetSingleData = () => {
        const val = { 'ChargeDegreeID': singleTypeId }
        fetchPostData('ChargeDegree/GetSingleData_ChargeDegree', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "ChargeDegreeCode": Editval[0]?.ChargeDegreeCode, 'IsActive':Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'ChargeDegreeID': Editval[0]?.ChargeDegreeID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'ChargeDegreeCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1',
                'ChargeDegreeID':''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'ChargeDegreeCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.ChargeDegreeCode === value.ChargeDegreeCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('ChargeDegree/InsertChargeDegree', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.ChargeDegreeID != singleTypeId) {
                if (item.ChargeDegreeCode === value.ChargeDegreeCode) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('ChargeDegree/UpdateChargeDegree', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.ChargeDegreeCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.ChargeDegreeCode) } })
        }
    ...
    return (
        <>
            {
                modal ?
                    <div class="modal fade" style={{ background: "rgba(0,0,0, 0.5)" }} id="ChargeDegreeModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-backdrop="false">
                        <div class="modal-dialog modal-lg modal-dialog-centered rounded" role="document">
                            <div class="modal-content">
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Charge Degree</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='ChargeDegreeCode' onChange={handlChanges} value={value.ChargeDegreeCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default ChargeDegree_Add_Up;
