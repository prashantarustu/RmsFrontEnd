...
import { OffMethodOFEntryListDropDownArray } from '../../../../Utility/ListDropDownArray/ListDropArray';
import FindListDropDown from '../../../../Common/FindListDropDown';
import Loader from '../../../../Common/Loader';
import { useContext } from 'react';
import { AgencyContext } from '../../../../../Context/Agency/Index';

const MethodOfEntry = ({ LoginPinID, LoginAgencyID, offenceID, MainIncidentID }) => {
  ...
  //       //  toastifyError("Data is already present")
  //     }
  //   }
  // }, [MethodOfOperationError])
  ...
  useEffect(() => {
    get_MethodOfEntry_DropDown(offenceID);
    get_MethodOfEntry_Data(offenceID);
    getScreenPermision(LoginAgencyID, LoginPinID);
  }, [offenceID])

  const getScreenPermision = (LoginAgencyID, LoginPinID) => {
    ScreenPermision("O040", LoginAgencyID, LoginPinID).then(res => {
      if (res) {
  ...
  const get_MethodOfEntry_Data = (offenceID) => {
    ...
    fetchPostData('OffenseMethodOfEntry/GetData_OffenseMethodOfEntry', val).then((res) => {
      if (res) {
        setMethodOfEntryData(res); setLoder(true)
      } else {
        setMethodOfEntryData([]); setLoder(true)
      }
    })
  }

  const get_MethodOfEntry_DropDown = (offenceID) => {
    const val = {
      'CrimeID': offenceID,
    }
  ...
  const AddMethodEntry = () => {
    AddDeleteUpadate('OffenseMethodOfEntry/Insert_OffenseMethodOfEntry', value).then((res) => {
      if (res) {
      ...
      cell: row => <>
        ...
        } */}
        {
          EffectiveScreenPermission ? EffectiveScreenPermission[0]?.DeleteOK ?
            <Link to={`#`} className="btn btn-sm bg-green text-white px-1 py-0 mr-1" onClick={() => { setMethodOfEntryId(row.MethodOfEntryID); }} data-toggle="modal" data-target="#DeleteModal">
  ...
  return (
    <>
      <div className="col-12 col-md-12 pt-2 p-0" >
        <div className="bg-line  py-1 px-2 d-flex justify-content-between align-items-center ">
          ...
          <div style={{ marginLeft: 'auto' }}>
            ...
            <FindListDropDown
              array={OffMethodOFEntryListDropDownArray}
            />
          </div>
        </div>
          ...
          <div className="col-6 col-md-6 col-lg-4 mt-2">
            <div className=" dropdown__box">
              <Select
                ...
                ref={SelectedValue}
              />
              <label htmlFor="">Method Of Entry</label>
        ...
        <div className="col-12">
          {
            loder ?
              <DataTable
                columns={columns}
                data={EffectiveScreenPermission ? EffectiveScreenPermission[0]?.DisplayOK ? methodOfEntryData : '' : methodOfEntryData}
                dense
                pagination
      ...
      <DeletePopUpModal func={DeleteMethodOfEntry} />
    </>
  )
}

export default MethodOfEntry