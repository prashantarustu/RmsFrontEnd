...
import PointOfExitAdd_Up from './PointOfExitAdd_Up';   
...
const PointOfexit = () => {
    ...
    const get_Data_List = () => {
        ...
        fetch_Post_Data('CrimePointOfexit/GetData_CrimePointOfexit', val)
            .then((res) => {
                if (res) {
    ...
    const UpdActiveDeactive = () => {
        const value = {
            'IsActive': IsActive,
            'PointOfExit': singleTypeId,
            'DeletedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'),
        }
        AddDeleteUpadate('CrimePointOfexit/DeleteCrimePointOfexit', value)
            .then(res => {
                if (res.success) {
    ...
    const columns = [
        {
            name: 'Code',
            selector: (row) => row.PointOfExitCode,
            sortable: true
        },
        {
            name: 'AgencyCode',
                ...
                <div style={{ position: 'absolute', top: 0, right: 40 }}>

                    {
                        pageStatus === "1" ?
                            EffectiveScreenPermission ? EffectiveScreenPermission[0]?.ChangeOK ?
                                <Link to="/ListManagement?page=Point Of Exit" data-toggle="modal" data-target="#BloodTypeModal" onClick={(e) => { setEditValue(e, row); }}
...

                    {
                        EffectiveScreenPermission ? EffectiveScreenPermission[0]?.DeleteOK ?
                            pageStatus === "1" ?
                                < Link to="/ListManagement?page=Point Of Exit" data-toggle="modal" data-target="#ConfirmModal" onClick={(e) => { setSingleTypeId(row.PointOfExit); setIsActive('0'); setConfirmType("InActive") }}
                                    className="btn btn-sm  text-white px-1 py-0 mr-1" style={{ background: "#ddd" }}>
                                    <i class="fa fa-toggle-on" style={{ color: "green" }} aria-hidden="true"></i>
                                </Link>
                                :
                                <Link to="/ListManagement?page=Point Of Exit" data-toggle="modal" data-target="#ConfirmModal" onClick={(e) => { setSingleTypeId(row.PointOfExit); setIsActive('1'); setConfirmType("Active") }}
                                    className="btn btn-sm  text-white px-1 py-0 mr-1" style={{ background: "#ddd" }}>
    ...
    const setEditValue = (e, row) => {
        setUpdateStatus(updateStatus + 1); setSingleTypeId(row.PointOfExit)
        setModal(true)
        setStatus(true);
    }
    ...
    return (
        <>
                    ...
                    <div className="row mt-2">
                        <div className="col-12 ">
                            <div className="bg-green text-white py-1 px-2 d-flex justify-content-between align-items-center">
                                <p className="p-0 m-0">Point Of Exit</p>
                                {
                                    pageStatus === '1' ?
                                        EffectiveScreenPermission ? EffectiveScreenPermission[0]?.AddOK ?
                                            <Link to="/ListManagement?page=Point Of Exit" className="text-white" onClick={setStatusFalse}
                    ...
                    <div className="row">
                        <div className="col-5">
                            <input type="text" onChange={(e) => {
                                ...
                                const result = Filter(DataList, e.target.value, searchValue2, filterTypeIdOption, 'PointOfExitCode', 'Description')
                        ...
                        <div className="col-5">
                            <input type="text" onChange={(e) => {
                                ...
                                const result = Filter(DataList, searchValue1, e.target.value, filterTypeDescOption, 'PointOfExitCode', 'Description')
                                setFilterDataList(result)
                            }}
            ...
            <PointOfExitAdd_Up {...{ singleTypeId, status, get_Data_List, DataList, modal, setModal, updateStatus }} />
