...
const Narrative = () => {
  ...
  useEffect(() => {
    if (localStoreArray) {
      if (localStoreArray?.AgencyID && localStoreArray?.PINID) {
        ...
        if (localStoreArray.ArrestID) { setArrestID(localStoreArray?.ArrestID); get_NarrativesData(localStoreArray?.ArrestID) } else { setArrestID('') }
        ...
        <div style={{ position: 'absolute', top: 4, right: 5 }}>

          {
            EffectiveScreenPermission ? EffectiveScreenPermission[0]?.DeleteOK ?
              <Link to={`#`} onClick={(e) => setArrestNarrativeID(row.ArrestNarrativeID)} className="btn btn-sm bg-green text-white px-1 py-0 mr-1" data-toggle="modal" data-target="#DeleteModal">
  ...
  const editNarratives = (e, val) => {
    ...
    setArrestNarrativeID(val.ArrestNarrativeID);
  ...
  return (
    <>
      ...
      <Narrative_Add_Up {...{ LoginPinID, ArrestID, LoginAgencyID, upDateCount, ArrestNarrativeID, status, modal, setModal, NarrativeData, get_NarrativesData }} />
    </>
  )
}
