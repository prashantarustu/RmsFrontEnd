...
const AgeUnitAdd_Up = (props) => {  
    ...
    const [value, setValue] = useState({
        'AgeUnitCode': '', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1', 'AgeUnitID': '', 'AgencyCode': ''
    });
    ...
    const GetSingleData = () => {
        const val = { 'AgeUnitID': singleTypeId }
        fetchPostData('AgeUnit/GetSingleData_AgeUnit', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "AgeUnitCode": Editval[0]?.AgeUnitCode, 'IsActive': Editval[0]?.IsActive, 'AgencyCode': Editval[0]?.AgencyCode,
                "Description": Editval[0]?.Description,
                'AgeUnitID': Editval[0]?.AgeUnitID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'AgeUnitCode': '', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1',
                'AgeUnitID': '', 'AgencyCode': ''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'AgeUnitCode': '', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName': '', 'AgencyCode': ''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.AgeUnitCode === value.AgeUnitCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('AgeUnit/InsertAgeUnit', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.AgeUnitID != singleTypeId) {
                if (item.AgeUnitCode === value.AgeUnitCode) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('AgeUnit/UpdateAgeUnit', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.AgeUnitCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.AgeUnitCode) } })
        }
                                ...
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Age Unit</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='AgeUnitCode' onChange={handlChanges} value={value.AgeUnitCode} className='requiredColor' />
                                                        <label>Code</label>
