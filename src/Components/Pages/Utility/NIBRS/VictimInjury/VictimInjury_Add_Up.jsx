...
const VictimInjury_Add_Up = (props) => {
    const { singleTypeId, status, get_Data_List, DataList, modal, setModal, updateStatus } = props
    const [agencyData, setAgencyData] = useState([])
    const [Editval, setEditval] = useState();

    const [value, setValue] = useState({
        'InjuryCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1', 'VictimInjuryID': '', 'IsFatalInjury': ''
    });
    ...
    const GetSingleData = () => {
        const val = { 'VictimInjuryID': singleTypeId }
        fetchPostData('VictimInjury/GetSingleData_VictimInjury', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "InjuryCode": Editval[0]?.InjuryCode, 'IsActive': Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'VictimInjuryID': Editval[0]?.VictimInjuryID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
                ...
                'ModifiedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'),
                 'IsFatalInjury': Editval[0]?.IsFatalInjury
            })
        }
        else {
            setValue({
                ...value,
                'InjuryCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1',
                'VictimInjuryID': '', 'IsFatalInjury':''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'InjuryCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName': '', 'IsFatalInjury':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const handlChanges = (e) => {
        if (e.target.name === 'IsFatalInjury') {
            setValue({
                ...value,
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.InjuryCode === value.InjuryCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('VictimInjury/InsertVictimInjury', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.VictimInjuryID != singleTypeId) {
                if (item.InjuryCode === value.InjuryCode) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            console.log(value);
            AddDeleteUpadate('VictimInjury/UpdateVictimInjury', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.InjuryCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.InjuryCode) } })
        }
    ...
    return (
        <>
            {
                modal ?
                    <div class="modal fade" style={{ background: "rgba(0,0,0, 0.5)" }} id="TypeModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-backdrop="false">
                        <div class="modal-dialog modal-lg modal-dialog-centered rounded" role="document">
                            <div class="modal-content">
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Victim Injury</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='InjuryCode' onChange={handlChanges} value={value.InjuryCode} className='requiredColor' />
                                                        <label>Code</label>
                                                ...
                                                {/* <div className="col-2 mt-3">
                                                    <input type="checkbox" name="IsFatalInjury" checked={value.IsFatalInjury} value={value.IsFatalInjury}
                                                        onChange={handlChanges}
                                                        disabled={''}
                                                        id="IsFatalInjury" />
                                                    <label className='ml-2' htmlFor="IsFatalInjury">IsFatalInjury</label>
                                                </div> */}
                                            </div>
                                        </fieldset>
                                    </div>
...
export default VictimInjury_Add_Up;
