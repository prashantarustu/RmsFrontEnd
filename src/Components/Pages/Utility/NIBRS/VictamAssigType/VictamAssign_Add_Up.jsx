...
const VictimAssign_Add_Up = (props) => {
    ...
    const [value, setValue] = useState({
        'AssignmentTypeCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1', 'VictimAssignmentTypeID': ''
    });
    ...
    const GetSingleData = () => {
        const val = { 'VictimAssignmentTypeID': singleTypeId }
        fetchPostData('VictimAssignmentType/GetSingleData_VictimAssignmentType', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "AssignmentTypeCode": Editval[0]?.AssignmentTypeCode,
                'IsActive': Editval[0]?.IsActive,
                'AgencyCode':Editval[0]?.AgencyCode,
                "Description": Editval[0]?.Description,
                'VictimAssignmentTypeID': Editval[0]?.VictimAssignmentTypeID,
            ...
            setValue({
                ...value,
                'AssignmentTypeCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1',
    ...
    const reset = () => {
        setValue({
            ...value,
            'AssignmentTypeCode': '', 'AgencyCode':'','Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName': ''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.AssignmentTypeCode === value.AssignmentTypeCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('VictimAssignmentType/InsertVictimAssignmentType', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.VictimAssignmentTypeID != singleTypeId) {
                if (item.AssignmentTypeCode === value.AssignmentTypeCode) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('VictimAssignmentType/UpdateVictimAssignmentType', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.AssignmentTypeCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.AssignmentTypeCode) } })
        }
    ...
    return (
        <>
            {  
                modal ?
                    <div class="modal fade" style={{ background: "rgba(0,0,0, 0.5)" }} id="VictamAsngTypeModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-backdrop="false">
                        <div class="modal-dialog modal-lg modal-dialog-centered rounded" role="document">
                            <div class="modal-content">
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Victim Officer Assignment Type</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='AssignmentTypeCode' onChange={handlChanges} value={value.AssignmentTypeCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default VictimAssign_Add_Up;
