...
const MartialStatus_Add_Up = (props) => {
    const { singleTypeId, status, get_Data_List, DataList, modal, setModal, updateStatus } = props
    const [agencyData, setAgencyData] = useState([])
    const [Editval, setEditval] = useState();

    const [value, setValue] = useState({
        'MaritalStatusCode': '', 'AgencyCode':'','Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 'MaritalStatusID':''
    });
    ...
    const GetSingleData = () => {
        const val = { 'MaritalStatusID': singleTypeId }
        fetchPostData('MaritalStatus/GetSingleData_MaritalStatus', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "MaritalStatusCode": Editval[0]?.MaritalStatusCode, 'IsActive':Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'MaritalStatusID': Editval[0]?.MaritalStatusID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'MaritalStatusCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1',
                'MaritalStatusID':''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'MaritalStatusCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.MaritalStatusCode === value.MaritalStatusCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('MaritalStatus/InsertMaritalStatus', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.MaritalStatusID != singleTypeId) {
                if (item.MaritalStatusCode === value.MaritalStatusCode) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('MaritalStatus/UpdateMaritalStatus', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.MaritalStatusCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.MaritalStatusCode) } })
        }
    ...
    return (
        <>
            {
                modal ?
                    <div class="modal fade" style={{ background: "rgba(0,0,0, 0.5)" }} id="MartialStatusModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-backdrop="false">
                        <div class="modal-dialog modal-lg modal-dialog-centered rounded" role="document">
                            <div class="modal-content">
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Martial Status</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='MaritalStatusCode' onChange={handlChanges} value={value.MaritalStatusCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default MartialStatus_Add_Up;
