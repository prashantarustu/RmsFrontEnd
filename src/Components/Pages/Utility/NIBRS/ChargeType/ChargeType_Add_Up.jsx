...
const ChargeType_Add_Up = (props) => {
    ...
    const [value, setValue] = useState({
        'ChargeTypeCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 'ChargeTypeID':''
    });
    ...
    const GetSingleData = () => {
        const val = { 'ChargeTypeID': singleTypeId }
        fetchPostData('ChargeType/GetSingleData_ChargeType', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "ChargeTypeCode": Editval[0]?.ChargeTypeCode, 'IsActive':Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'ChargeTypeID': Editval[0]?.ChargeTypeID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'ChargeTypeCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1',
                'ChargeTypeID':''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'ChargeTypeCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.ChargeTypeCode === value.ChargeTypeCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('ChargeType/InsertChargeType', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.ChargeTypeID != singleTypeId) {
                if (item.ChargeTypeCode === value.ChargeTypeCode) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('ChargeType/UpdateChargeType', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.ChargeTypeCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.ChargeTypeCode) } })
        }
                                ...
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Charge Type</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='ChargeTypeCode' onChange={handlChanges} value={value.ChargeTypeCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default ChargeType_Add_Up;
