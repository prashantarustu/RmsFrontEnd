...
import SMTType_Add_Up from './SMTType_Add_Up';
...
const SMTType = () => {
  ...
  const get_data_PropertyDescription = () => {
    ...
    fetch_Post_Data('SMTTypes/GetData_SMTTypes', val)
      .then((res) => {
        if (res) {
  ...
  const [SMTTypeID, setSMTTypeID] = useState('')
  ...
  const UpdActiveDeactive = () => {
    ...
    AddDeleteUpadate('SMTTypes/DeleteSMTTypes', value)
      .then(res => {
        if (res.success) {
        ...
        <div style={{ position: 'absolute', top: 0, right: 40 }}>

          {
            pageStatus === "1" ?
              EffectiveScreenPermission ? EffectiveScreenPermission[0]?.ChangeOK ?
                <Link to="/ListManagement?page=SMT Type" data-toggle="modal" data-target="#PropertyDescriptionCodes" onClick={(e) => { setEditValue(e, row) }}
          ...
          }
          {
            EffectiveScreenPermission ? EffectiveScreenPermission[0]?.DeleteOK ?
              pageStatus === "1" ?
                < Link to="/ListManagement?page=SMT Type" data-toggle="modal" data-target="#ConfirmModal" onClick={(e) => { setSMTTypeID(row.SMTTypeID); setIsActive('0'); setConfirmType("InActive") }}
                  className="btn btn-sm  text-white px-1 py-0 mr-1" style={{ background: "#ddd" }}>
                  <i class="fa fa-toggle-on" style={{ color: "green" }} aria-hidden="true"></i>
                </Link>
                :
                <Link to="/ListManagement?page=SMT Type" data-toggle="modal" data-target="#ConfirmModal" onClick={(e) => { setSMTTypeID(row.SMTTypeID); setIsActive('1'); setConfirmType("Active") }}
                  className="btn btn-sm  text-white px-1 py-0 mr-1" style={{ background: "#ddd" }}>
  ...
  const setEditValue = (e, row) => {
    ...
    setModal(true); setStatus(true); setSMTTypeID(row.SMTTypeID)
  }
  ...
  return (
    ...
    <>
          ...
          <div className="row mt-2">
            <div className="col-12 ">
              <div className="bg-green text-white py-1 px-2 d-flex justify-content-between align-items-center">
                <p className="p-0 m-0">SMT Type</p>
                {
                  pageStatus === '1' ?
                    EffectiveScreenPermission ? EffectiveScreenPermission[0]?.AddOK ?
                      <Link to="/ListManagement?page=SMT Type" className="text-white" onClick={setStatusFalse}
          ...
          <div className="row">
            <div className="col-5">
              <input type="text" onChange={(e) => {
                ...
                const result = Filter(PropertyDescriptionData, e.target.value, searchValue2, filterPropertyDescriptionCode, 'SMTTypeCode', 'Description')
            ...
            <div className="col-5">
              <input type="text" onChange={(e) => {
                ...
                const result = Filter(PropertyDescriptionData, searchValue1, e.target.value, filterPropertyDescription, 'SMTTypeCode', 'Description')
                setPropertyDescriptionFilterData(result)
              }}
      ...
      {/* <Criminal_Add_Up {...{ SMTTypeID, status, get_data_CrimeBias, PropertyDescriptionData, modal, setModal, PropertyDescriptionStatus }} /> */}
      <SMTType_Add_Up {...{ SMTTypeID, status, get_data_PropertyDescription, PropertyDescriptionData, modal, setModal, PropertyDescriptionStatus }} />
      <ConfirmModal func={UpdActiveDeactive} confirmType={confirmType} />
    </>
  )
}

export default SMTType