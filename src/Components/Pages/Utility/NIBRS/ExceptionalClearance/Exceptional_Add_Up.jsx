...
const Exceptional_Add_Up = (props) => {  

    const { ClearanceID, status, get_data_Exceptional_Type, ExceptionalTypeData, modal, setModal, ExceptionalTypeupdStatus } = props
    const [agencyData, setAgencyData] = useState([])
    const [Editval, setEditval] = useState();

    const [value, setValue] = useState({
        'ClearanceCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1'
    });
    ...
    }, [ClearanceID, ExceptionalTypeupdStatus])

    const GetSingleData = () => {
        const val = { 'ClearanceID': ClearanceID }
        fetchPostData('ExceptionalClearance/GetSingleData_ExceptionalClearance', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "ClearanceCode": Editval[0]?.ClearanceCode, 'IsActive':Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'ClearanceID': Editval[0]?.ClearanceID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'ClearanceCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1'
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'ClearanceCode': '', 'Description': '', 'AgencyCode':'','AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = ExceptionalTypeData?.find(item => {
            if (item.ClearanceCode === value.ClearanceCode) {
                return true
            } else return false
        }
        );
        var result1 = ExceptionalTypeData?.find(item => {
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('ExceptionalClearance/InsertExceptionalClearance', value).then((res) => {
    ...
    const update_Type = () => {
        var result = ExceptionalTypeData?.find(item => {
            if (item.ClearanceID != ClearanceID) {
                if (item.ClearanceCode === value.Description) {
                    return true
                } else return false
            }
        }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('ExceptionalClearance/UpdateExceptionalClearance', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.ClearanceCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.ClearanceCode) } })
        }
    ...
    return (
        <>
            {
                modal ?
                    <div class="modal fade" style={{ background: "rgba(0,0,0, 0.5)" }} id="ExceptionModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-backdrop="false">
                        <div class="modal-dialog modal-lg modal-dialog-centered rounded" role="document">
                            <div class="modal-content">
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Cleared Exceptionally</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='ClearanceCode' onChange={handlChanges} value={value.ClearanceCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default Exceptional_Add_Up;
