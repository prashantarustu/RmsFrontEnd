...
const RecoveredProperty_Add_Up = (props) => {
    ...
    const { modal, setModal, VehicleRecoveredID, setVehicleRecoveredID, updateStatus, get_property_Data, vehicleData, LoginPinID, LoginAgencyID, vehicleID, MainIncidentID } = props
    ...
    useEffect(() => {
        if (LoginAgencyID) {
            get_Head_Of_Agency(LoginAgencyID); get_RecoveryType(LoginAgencyID); get_UCRRecovered(LoginAgencyID); get_Dispositions(LoginAgencyID);
        }
    }, [LoginAgencyID])
    ...
    const Add_RecoveredProperty = (e) => {
        if (value.RecoveredValue != 0) {
            if (parseFloat(value.RecoveredValue) <= parseFloat(remainBalance)) {
                if (remainBalance !== 0) {
                    AddDeleteUpadate('VehicleRecovered/Insert_VehicleRecovered', value)
                        .then((res) => {
                                    ...
                                    <div class="modal-body">
                                        <div className="m-1">
                                            <fieldset style={{ border: '1px solid gray' }}>
                                                <legend style={{ fontWeight: 'bold' }}>Vehicle Recovered</legend>
                                                <div className="row">
                                                    <div className="col-12">
                                                        <div className="row bb">
                                                            ...
                                                            <div className="col-6 col-md-6 col-lg-4 pt-2">
                                                                <div class="text-field">
                                                                    <input type="text" maxLength={19} name="RecoveredValue" value={value?.RecoveredValue} onChange={HandleChanges}
                                                                        className={`${recoverTypeCode === 'FU' || recoverTypeCode === 'F' || recoverTypeCode === '' ? "readonlyColor" : ""} requiredColor`} required
                                                                        readOnly={recoverTypeCode === 'FU' || recoverTypeCode === 'F' || recoverTypeCode === '' ? true : false} pattern="[^.]{0,1}"
                                                                    />
