...
const Type_Add_Up = (props) => {
    const { singleTypeId, status, get_Data_List, DataList, modal, setModal, updateStatus } = props
    const [agencyData, setAgencyData] = useState([])
    const [Editval, setEditval] = useState();

    const [value, setValue] = useState({
        'TypeCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1', 'TypeID': ''
    });
    ...
    const GetSingleData = () => {
        const val = { 'TypeID': singleTypeId }
        fetchPostData('Type/GetSingleData_Type', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "TypeCode": Editval[0]?.TypeCode, 'IsActive': Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'TypeID': Editval[0]?.TypeID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'TypeCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1',
                'TypeID': ''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'TypeCode': '', 'Description': '', 'AgencyCode':'','AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName': ''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('Type/InsertType', value).then((res) => {
    ...
    const update_Type = () => {
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('Type/UpdateType', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        ...
        if (RequiredField(value.TypeCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.TypeCode) } })
        }
                                            ...
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='TypeCode' onChange={handlChanges} value={value.TypeCode} className='requiredColor' />
                                                        <label>Code</label>
