...
const Home = (props) => {
    ...
    const { aId, pinId, allowMultipleLogin, send_Notification } = props

    // Hooks Initialization
    const { agencyData, getAgency, get_CountList, setStatus, status, inActiveStatus, setInActiveStatus, changesStatus, setChangesStatus, changesStatusCount, getInActiveAgency, localStoreArray, setLocalStoreArray, get_LocalStorage, setAgencyName } = useContext(AgencyContext);
    ...
    useEffect(() => {
        if (agencyEditVal[0]?.AgencyID) {
            // console.log(agencyEditVal)
            // console.log(agencyEditVal[0]?.IsActive)
            //-------------------comment on 12-12-2023
            // getScreenPermision(agencyEditVal[0]?.AgencyID, pinId)
            // get_Field_Permision_Agency(agencyEditVal[0]?.AgencyID, pinId)
            setValue({
                ...value,
    ...
    const agencySubmit = () => {
        ...
        if (result) {
            ...
            axios.post('Agency/AgencyInsert', data).then(response => {
                ...
                if (response.data.success) {
                    ...
                    // send_Notification({
                    //     data:{
                    //         'ORI': value.ORI, 'MunicipalityCode': value.MunicipalityCode, 'Agency_Name': value.Agency_Name, 'ShortName': value.ShortName, 'Agency_Address1': value.Agency_Address1, 'Agency_StateId': value.Agency_StateId, 'Agency_CityId': value.Agency_CityId, 'Agency_ZipId': value.Agency_ZipId, 'Agency_Phone': value.Agency_Phone, 'Agency_Fax': value.Agency_Fax, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': value.ModifiedByUserFK, 'AgencyID': response?.data?.Id, 'ZipName': value.ZipName, 'StateName': value.StateName, 'CityName': value.CityName, 'IsActive': true,
                    //     },
            ...
            axios.post('Agency/AgencyUpdate', data)
                .then(response => {
                    if (response.data.success) {
                        ...
                        // if (inActiveStatus === true && value.IsActive === true) { getInActiveAgency(); setStatus(true) }
                                            ...
                                            EffectiveScreenPermission ?
                                                EffectiveScreenPermission[0]?.Changeok ?
                                                    <>
                                                        ...
                                                        {/* <button type="button" className="btn btn-sm btn-success" data-toggle="modal" data-target={changesStatus?"#SaveModal":''} >Close</button> */}
