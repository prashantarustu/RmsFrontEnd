...
const Comments = () => {
    ...
    if (localStoreArray) {
      if (localStoreArray?.AgencyID && localStoreArray?.PINID) {
        ...
        if (localStoreArray?.WarrantID) {
          setWarrentID(localStoreArray?.WarrantID);
  ...
  return (
    <>
      ...
      <Comments_Add_Up {...{ userName, upDateCount, WarrantCommentsID, status, modal, setModal, get_CommentsData, CommentData, LoginPinID, LoginAgencyID, WarrantID, }} />
    </>
  )
}
