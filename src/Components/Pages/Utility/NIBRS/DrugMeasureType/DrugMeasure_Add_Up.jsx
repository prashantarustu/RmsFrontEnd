...
const DrugMeasure_Add_Up = (props) => {  

    const { DrugMeasureTypeID, status, get_data_List, DrugMeasureData, modal, setModal, updStatus } = props
    // const { DrugMeasureTypeID, status, get_data_List, DrugMeasureData, modal, setModal, updStatus } = props
    ...
    const [value, setValue] = useState({
        'DrugMeasureTypeCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1'
    });
    ...
    useEffect(() => {
        if (DrugMeasureTypeID) {
            GetSingleData()
        }
    }, [DrugMeasureTypeID, updStatus])

    const GetSingleData = () => {
        const val = { 'DrugMeasureTypeID': DrugMeasureTypeID }
        fetchPostData('PropertyDrugMeasureType/GetSingleData_PropertyDrugMeasureType', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "DrugMeasureTypeCode": Editval[0]?.DrugMeasureTypeCode, 'IsActive':Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'DrugMeasureTypeID': Editval[0]?.DrugMeasureTypeID,
            ...
            setValue({
                ...value,
                'DrugMeasureTypeCode': '', 'AgencyCode':'','Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1'
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'DrugMeasureTypeCode': '', 'Description': '', 'AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DrugMeasureData?.find(item => {
            if (item.DrugMeasureTypeCode === value.DrugMeasureTypeCode) {
                return true
            } else return false
        }
        );
        var result1 = DrugMeasureData?.find(item => {
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('PropertyDrugMeasureType/InsertPropertyDrugMeasureType', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DrugMeasureData?.find(item => {
            if (item.DrugMeasureTypeID != DrugMeasureTypeID) {
                if (item.DrugMeasureTypeCode === value.Description) {
                    return true
                } else return false
            }
        }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('PropertyDrugMeasureType/UpdatePropertyDrugMeasureType', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.DrugMeasureTypeCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.DrugMeasureTypeCode) } })
        }
    ...
    return (
        <>
            {
                modal ?
                    <div class="modal fade" style={{ background: "rgba(0,0,0, 0.5)" }} id="DrugMeasureModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-backdrop="false">
                        <div class="modal-dialog modal-lg modal-dialog-centered rounded" role="document">
                            <div class="modal-content">
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Type Drug Measurement</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='DrugMeasureTypeCode' onChange={handlChanges} value={value.DrugMeasureTypeCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default DrugMeasure_Add_Up;
