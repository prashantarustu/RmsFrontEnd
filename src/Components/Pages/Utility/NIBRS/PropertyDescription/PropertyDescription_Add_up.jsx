...
const PropertyDescription_Add_Up = (props) => {

    const { PropertyDescID, status, get_data_PropertyDescription, PropertyDescriptionData, modal, setModal, PropertyDescriptionStatus } = props
    ...
    }, [PropertyDescID, PropertyDescriptionStatus])
    // GetSingledataPropertyDescription

    const GetSingledataPropertyDescription = () => {
        const val = { 'PropertyDescID': PropertyDescID }
        fetchPostData('PropertyDescriptionCodes/GetSingleData_PropertyDescriptionCodes', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "PropDescCode": PropertyDescriptionEditVal[0]?.PropDescCode,
                ...
                'PropertyDescID': PropertyDescriptionEditVal[0]?.PropertyDescID,
    ...
    const Add_PropertyDescription = (e) => {
        var result = PropertyDescriptionData?.find(item => {
            if (item.PropDescCode === value.PropDescCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            if (result) {
                ...
                setErrors({ ...errors, ['PropDescCodeError']: '' })
            }
            if (result1) {
            ...
            AddDeleteUpadate('PropertyDescriptionCodes/InsertPropertyDescriptionCodes', value).then((res) => {
    ...
    const Update_PropertyDescription = () => {
        var result = PropertyDescriptionData?.find(item => {
            if (item.PropertyDescID != PropertyDescID) {
                if (item.PropDescCode === value.PropDescCode) {
                    return true
                } else return false
            }
        ...
        var result1 = PropertyDescriptionData?.find(item => {
            if (item.PropertyDescID != value.PropertyDescID) {
                if (item.Description === value.Description) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('PropertyDescriptionCodes/UpdatePropertyDescriptionCodes', value).then((res) => {
                toastifySuccess(res.Message);
                setErrors({ ...errors, ['PropDescCodeError']: '' })
                get_data_PropertyDescription();
                setModal(false)
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.PropDescCode)) {
            setErrors(prevValues => { return { ...prevValues, ['PropDescCodeError']: RequiredField(value.PropDescCode) } })
        }
    ...
    const { DescriptionError, PropDescCodeError } = errors

    useEffect(() => {
        if (DescriptionError === 'true' && PropDescCodeError === 'true') {
            if (status) Update_PropertyDescription()
            else Add_PropertyDescription()
        }
    }, [DescriptionError, PropDescCodeError])
    ...
    return (
        <>
            {
                modal ?
                    <div class="modal fade" style={{ background: "rgba(0,0,0, 0.5)" }} id="PropertyDescriptionCodes" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-backdrop="false">
                        <div class="modal-dialog modal-lg modal-dialog-centered rounded" role="document">
                            <div class="modal-content">
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Property Description</legend>

                                            <div className="row ">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" name='PropDescCode' maxLength={10} onChange={handlChanges} value={value.PropDescCode} className='requiredColor' />
                                                        <label>Code</label>
                                                        {errors.PropDescCodeError !== 'true' ? (
                                                            <span style={{ color: 'red', fontSize: '13px', margin: '0px', padding: '0px' }}>{errors.PropDescCodeError}</span>
                                                        ) : null}
                                                    </div>
                                                </div>
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
...
export default PropertyDescription_Add_Up
