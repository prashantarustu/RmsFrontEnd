...
const ConditionType_Add_Up = (props) => {
    const { singleTypeId, status, get_Data_List, DataList, modal, setModal, updateStatus } = props
    const [agencyData, setAgencyData] = useState([])
    const [Editval, setEditval] = useState();

    const [value, setValue] = useState({
        'ConditionCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1', 'ConditionId': '', 'IsMental': ''
    });
    ...
    const GetSingleData = () => {
        const val = { 'ConditionId': singleTypeId }
        fetchPostData('ConditionType/GetSingleData_ConditionType', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "ConditionCode": Editval[0]?.ConditionCode, 'IsActive': Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'ConditionId': Editval[0]?.ConditionId, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'ConditionCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1',
                'ConditionId': '', 'IsMental':''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'ConditionCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName': '', 'IsMental':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.ConditionCode === value.ConditionCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('ConditionType/InsertConditionType', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.ConditionId != singleTypeId) {
                if (item.ConditionCode === value.ConditionCode) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('ConditionType/UpdateConditionType', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.ConditionCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.ConditionCode) } })
        }
                                ...
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Condition Type</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='ConditionCode' onChange={handlChanges} value={value.ConditionCode} className='requiredColor' />
                                                        <label>Code</label>
                                                ...
                                                {/* <div className="col-4 mt-3">
                                                    <input type="checkbox" name="IsMental" checked={value.IsMental} value={value.IsMental}
                                                        ...
                                                        disabled={''}
                                                        id="IsMental" />
                                                    <label className='ml-2' htmlFor="IsMental">IsMental</label>
                                                </div> */}
                                            </div>
                                        </fieldset>
                                    </div>
...
export default ConditionType_Add_Up;
