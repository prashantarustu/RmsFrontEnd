...
const IncidentScence_Add_Up = (props) => {   
    ...
    const [value, setValue] = useState({
        'SceneCode': '', 'Description': '', 'AgencyId': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 'SceneID':'', 'AgencyCode':''
    });
    ...
    const GetSingleData = () => {
        const val = { 'SceneID': singleTypeId }
        fetchPostData('IncidentScene/GetSingleData_IncidentScene', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "SceneCode": Editval[0]?.SceneCode, 'IsActive':Editval[0]?.IsActive, 'AgencyCode':Editval[0]?.AgencyCode,
                "Description": Editval[0]?.Description,
                'SceneID': Editval[0]?.SceneID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'SceneCode': '', 'Description': '', 'AgencyId': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 'AgencyCode':'', 'SceneID':''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'SceneCode': '', 'Description': '', 'AgencyId': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName':'', 'AgencyCode':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.SceneCode === value.SceneCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('IncidentScene/InsertIncidentScene', value).then((res) => {
    ...
    const update_Type = () => {
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('IncidentScene/UpdateIncidentScene', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.SceneCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.SceneCode) } })
        }
                                ...
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Incident Scene</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" name='SceneCode' onChange={handlChanges} value={value.SceneCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default IncidentScence_Add_Up;
