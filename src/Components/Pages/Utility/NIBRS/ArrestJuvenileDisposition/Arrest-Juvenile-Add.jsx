...
const Arrest_Juv_Add_Up = (props) => {
    ...
    const [value, setValue] = useState({
        'ArrestJuvenileDispositionCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 'ArrestJuvenileDispositionID':''
    });
    ...
    const GetSingleData = () => {
        const val = { 'ArrestJuvenileDispositionID': singleTypeId }
        fetchPostData('ArrestJuvenileDisposition/GetSingleData_ArrestJuvenileDisposition', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "ArrestJuvenileDispositionCode": Editval[0]?.ArrestJuvenileDispositionCode, 'IsActive':Editval[0]?.IsActive,
                "Description": Editval[0]?.Description, 'AgencyCode':Editval[0]?.AgencyCode,
                'ArrestJuvenileDispositionID': Editval[0]?.ArrestJuvenileDispositionID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'ArrestJuvenileDispositionCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 
                'ArrestJuvenileDispositionID':''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'ArrestJuvenileDispositionCode': '', 'AgencyCode':'','Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.ArrestJuvenileDispositionCode === value.ArrestJuvenileDispositionCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('ArrestJuvenileDisposition/InsertArrestJuvenileDisposition', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.ArrestJuvenileDispositionID != singleTypeId) {
                if (item.ArrestJuvenileDispositionCode === value.ArrestJuvenileDispositionCode) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('ArrestJuvenileDisposition/UpdateArrestJuvenileDisposition', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.ArrestJuvenileDispositionCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.ArrestJuvenileDispositionCode) } })
        }
                                ...
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Arrest Juvenile Disposition</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10}  name='ArrestJuvenileDispositionCode' onChange={handlChanges} value={value.ArrestJuvenileDispositionCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default Arrest_Juv_Add_Up;
