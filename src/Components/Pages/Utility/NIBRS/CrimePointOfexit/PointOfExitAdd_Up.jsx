...
const PointOfExitAdd_Up = (props) => {   
    ...
    const [value, setValue] = useState({
        'PointOfExitCode': '', 'AgencyCode':'','Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1', 'PointOfExit': ''
    });
    ...
    const GetSingleData = () => {
        const val = { 'PointOfExit': singleTypeId }
        fetchPostData('CrimePointOfexit/GetSingleData_CrimePointOfexit', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "PointOfExitCode": Editval[0]?.PointOfExitCode, 'IsActive': Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'PointOfExit': Editval[0]?.PointOfExit, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'PointOfExitCode': '','AgencyCode':'', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName': '', 'IsActive': '1',
                'PointOfExit': ''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'PointOfExitCode': '', 'Description': '', 'AgencyCode':'','AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName': ''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.PointOfExitCode === value.PointOfExitCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('CrimePointOfexit/InsertCrimePointOfexit', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.PointOfExit != singleTypeId) {
                if (item.PointOfExitCode === value.PointOfExitCode) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('CrimePointOfexit/UpdateCrimePointOfexit', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.PointOfExitCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.PointOfExitCode) } })
        }
                                ...
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Point Of Exit</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text"  maxLength={10} name='PointOfExitCode' onChange={handlChanges} value={value.PointOfExitCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default PointOfExitAdd_Up;
