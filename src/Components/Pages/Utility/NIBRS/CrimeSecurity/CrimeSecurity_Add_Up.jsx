...
const CrimeSecurity_Add_Up = (props) => {
    const { singleTypeId, status, get_Data_List, DataList, modal, setModal, updateStatus } = props
    const [agencyData, setAgencyData] = useState([])
    const [Editval, setEditval] = useState();

    const [value, setValue] = useState({
        'SecurityviolatedCode': '', 'AgencyCode':'','Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 'SecurityviolatedID':''
    });
    ...
    const GetSingleData = () => {
        const val = { 'SecurityviolatedID': singleTypeId }
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "SecurityviolatedCode": Editval[0]?.SecurityviolatedCode, 'IsActive':Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'SecurityviolatedID': Editval[0]?.SecurityviolatedID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'SecurityviolatedCode': '', 'AgencyCode':'','Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1',
                'SecurityviolatedID':''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'SecurityviolatedCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.SecurityviolatedID != singleTypeId) {
                if (item.SecurityviolatedCode === value.SecurityviolatedCode) {
                    return true
                } else return false
            }
    ...
    const check_Validation_Error = (e) => {
        ...
        if (RequiredField(value.SecurityviolatedCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.SecurityviolatedCode) } })
        }
                                ...
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Crime Security</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='SecurityviolatedCode' onChange={handlChanges} value={value.SecurityviolatedCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default CrimeSecurity_Add_Up;
