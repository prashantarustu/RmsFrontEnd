...
const Home = (props) => {
  ...
  const { ArresteName, ArrestID, get_Arrest_Count, get_ArrestCharge_Count, setChangesStatus, setIncStatus, incidentNumber, setIncidentNumber, updateCount, setUpdateCount, localStoreArray, setLocalStoreArray, get_LocalStorage, deleteStoreData, storeData } = useContext(AgencyContext);
  ...
  useEffect(() => {
    if (localStoreArray) {
      if (localStoreArray?.AgencyID && localStoreArray?.PINID) {
        ...
        setValue({ ...value, 'IncidentID': localStoreArray?.IncidentID, 'ArrestID': localStoreArray?.ArrestID, 'CreatedByUserFK': localStoreArray?.PINID, 'AgencyID': localStoreArray?.AgencyID, 'Name': localStoreArray?.ArresteeName, 'NameID': localStoreArray?.ArresteeID, }
        )
        if (localStoreArray.ArrestNumber) { setArrestNumber(localStoreArray.ArrestNumber) } else { setArrestNumber('') }
        if (localStoreArray.IncidentID) { setMainIncidentID(localStoreArray?.IncidentID); } else { setMainIncidentID('') }
        if (localStoreArray.ChargeID) { setChargeID(localStoreArray?.ChargeID); } else { setChargeID('') } get_ArrestCharge_Count(localStoreArray?.ChargeID)
      }
    }
  }, [localStoreArray])
  ...
  const OnClose = () => {
    ...
    deleteStoreData({ 'ChargeID': '',  'ArrestChargeStatus': '', 'ArresteeName': '', 'ArresteeID': '', });
  }
    ...
    <>
      <div className="col-12 col-md-12 pt-2 p-0" >
        ...
        <div className="row ">
          ...
          <div className="col-3  col-md-3 col-lg-3  mt-1 pt-1" >
            <div className="text-field">
              <input type="text" className='readonlyColor' name='ArrestNumber'
                value={arrestNumber ? arrestNumber : ''}
                required readOnly />
