...
const TeethCode_Add_Up = (props) => {
    ...
    const [value, setValue] = useState({
        'TeethCode': '', 'AgencyCode':'','Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 'TeethID':''
    });
    ...
    const GetSingleData = () => {
        const val = { 'TeethID': singleTypeId }
        fetchPostData('TeethCodes/GetSingleData_TeethCodes', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "TeethCode": Editval[0]?.TeethCode, 'IsActive':Editval[0]?.IsActive,
                "Description": Editval[0]?.Description,
                'AgencyCode':Editval[0]?.AgencyCode,
                'TeethID': Editval[0]?.TeethID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'TeethCode': '', 'Description': '', 'AgencyCode':'','AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1',
                'TeethID':''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'TeethCode': '', 'Description': '','AgencyCode':'', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        var result = DataList?.find(item => {
            if (item.TeethCode === value.TeethCode) {
                return true
            } else return false
        }
        );
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('TeethCodes/InsertTeethCodes', value).then((res) => {
    ...
    const update_Type = () => {
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('TeethCodes/UpdateTeethCodes', value).then((res) => {
    ...
    const check_Validation_Error = (e) => {
        e.preventDefault()
        if (RequiredField(value.TeethCode)) {
            setErrors(prevValues => { return { ...prevValues, ['CodeError']: RequiredField(value.TeethCode) } })
        }
                                ...
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Teeth Code</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" maxLength={10} name='TeethCode' onChange={handlChanges} value={value.TeethCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default TeethCode_Add_Up;
