...
const Penalties = () => {
    ...
    const { setIncStatus, updateCount, setUpdateCount, get_LocalStorage, localStoreArray } = useContext(AgencyContext);
    ...
    useEffect(() => {
        if (localStoreArray) {
            if (localStoreArray?.AgencyID && localStoreArray?.PINID) {
                ...
                if (localStoreArray.ChargeID) { setArrestChargeID(localStoreArray?.ChargeID); } else { setArrestChargeID('') }
            }
        }
    }, [localStoreArray]);
    ...
    useEffect(() => {
        if (ArrestChargeID) {
            setValue({ ...value, 'ChargeID': ArrestChargeID, 'CreatedByUserFK': LoginPinID })
        }
    }, [ArrestChargeID, LoginPinID]);
    ...
    const Sum = () => {
        ...
        // var substring = total?.substring(0, index + 3);
        setValue({
            ...value,
            // ['TotalPenalty']: `\$ ${total}`
            ['TotalPenalty']: total
