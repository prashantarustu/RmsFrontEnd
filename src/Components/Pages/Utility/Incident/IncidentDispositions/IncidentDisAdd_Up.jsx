...
const IncidentDisAdd_Up = (props) => {
    ...
    const [value, setValue] = useState({
        'DispositionCode': '', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'CreatedByUserFK': Decrypt_Id_Name(localStorage.getItem('PINID'), 'UForUserID'), 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 'IncidentDispositionsID':'', 'AgencyCode':''
    });
    ...
    const GetSingleData = () => {
        const val = { 'IncidentDispositionsID': singleTypeId }
        fetchPostData('IncidentDispositions/GetSingleData_IncidentDispositions', val)
            .then((res) => {
    ...
    useEffect(() => {
        if (status) {
            setValue({
                ...value,
                "DispositionCode": Editval[0]?.DispositionCode, 'IsActive':Editval[0]?.IsActive, 'AgencyCode':Editval[0]?.AgencyCode,
                "Description": Editval[0]?.Description,
                'IncidentDispositionsID': Editval[0]?.IncidentDispositionsID, 'MultiAgency_Name': Editval[0]?.MultiAgency_Name,
            ...
            setValue({
                ...value,
                'DispositionCode': '', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'ModifiedByUserFK': '', 'AgencyName':'', 'IsActive':'1', 'AgencyCode':'', 'IncidentDispositionsID':''
            })
        }
    }, [Editval])
    ...
    const reset = () => {
        setValue({
            ...value,
            'DispositionCode': '', 'Description': '', 'AgencyID': '', 'MultiAgency_Name': '', 'IsEditable': 0, 'ModifiedByUserFK': '', 'AgencyName':'', 'AgencyCode':''
        })
        setErrors({
            'CodeError': '',
            'DescriptionError': '',
    ...
    const Add_Type = (e) => {
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('IncidentDispositions/InsertIncidentDispositions', value).then((res) => {
    ...
    const update_Type = () => {
        var result = DataList?.find(item => {
            if (item.IncidentDispositionsID != singleTypeId) {
                if (item.DispositionCode === value.DispositionCode) {
                    return true
                } else return false
            }
        ...
        if (result || result1) {
            ...
            AddDeleteUpadate('IncidentDispositions/UpdateIncidentDispositions', value).then((res) => {
                                ...
                                <div class="modal-body">
                                    <div className="m-1 mt-3">
                                        <fieldset style={{ border: '1px solid gray' }}>
                                            <legend style={{ fontWeight: 'bold' }}>Incident Dispositions</legend>
                                            <div className="row">
                                                <div className="col-12 col-md-2 col-lg-2 mt-2">
                                                    <div class="text-field">
                                                        <input type="text" name='DispositionCode' onChange={handlChanges} value={value.DispositionCode} className='requiredColor' />
                                                        <label>Code</label>
...
export default IncidentDisAdd_Up;
